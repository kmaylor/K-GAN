...
class GGAN(object):
    """ Class for quickly building a GGAN model, randomly interpolates between generated data and
        real data after each upsampling in the generator.
    ...
    def build_generator(self): #make generator take downsampled image at each upsample and average
        '''Create the generator'''

        def generator_block(G, real_down, depth, kernel, stride, i, layer = 'mid'):

            def random_average():
                def layer(x):
                    weights = K.random_uniform((K.shape(x[0])[0],1,1,1))
                    return (weights * .1*x[0]) + ( x[1])
                return Lambda(layer)

            if layer == 'first':
                G = random_average()([real_down,G])
                G = BatchNormalization(momentum=0.9, name = 'BNorm_G1')(G)
                G = LeakyReLU(alpha=0.2, name = 'LRelu_G1')(G)
            else:
                G = UpSampling2D(stride,name='UpSample_%i'%(i+1), interpolation='bilinear')(G)
                G = random_average()([real_down,G])
                if layer == 'mid':
                    G = Conv2D(depth, kernel, strides = 1, padding='same',
                        kernel_initializer='he_normal',bias_initializer='zeros', name = 'Conv2D_G%i'%(i+1))(G)
                    G = BatchNormalization(momentum=0.9, name = 'BN_G%i'%(i+2))(G)
                    G = LeakyReLU(alpha=0.2, name = 'LRelu_G%i'%(i+2))(G)                
                else:
                    G = Conv2D(depth, kernel, strides = 1, padding='same',
                        kernel_initializer='glorot_normal',bias_initializer='zeros', name = 'Conv2D_G%i'%(i+1))(G)
                    G = Activation('tanh', name = 'Tanh')(G)
            return G

        real_img = Input(shape=(self.img_rows, self.img_cols, self.channel))
        latent = Input(shape=(self.latent_dim,))
        
        real_downsamples = [None for i in range(len(self.strides))]
        real_downsamples[0] = MaxPooling2D(self.strides[0],padding='same')(real_img)
        for i,s in enumerate(self.strides[1:]):
            real_downsamples[i+1] = MaxPooling2D(pool_size=s,padding='same')(real_downsamples[i])
        ...
        G = Dense(dim1*dim2*depth*depth_scale[0], input_dim=self.latent_dim,
                        kernel_initializer='he_normal',bias_initializer='zeros', name = 'Dense_G')(latent)
        
        G = Reshape((dim1, dim2, depth*depth_scale[0]),name='Reshape')(G)
        G = generator_block(G,real_downsamples[::-1][0],depth*depth_scale[0],1,1,1, layer='first')

        # Iterate over layers defined by the number of kernels and strides
        #Use larger kernels with larger feature maps
        for i,ks in enumerate(zip(self.kernels[:-1],self.strides[:-1])):
           G = generator_block(G,real_downsamples[::-1][i+1],depth*depth_scale[i+1],ks[0],ks[1],i+1)
        
        G = generator_block(G,real_img,self.channel,self.kernels[-1],self.strides[-1],i+2,layer='last')
        # If the output of the last layer is larger than the input for the discriminator crop
        # the image
        crop_r = int((K.int_shape(G)[1]-self.img_rows)/2)
        crop_c = int((K.int_shape(G)[2]-self.img_cols)/2)
        G = Cropping2D(cropping=((crop_c,crop_c),(crop_r,crop_r)), name = 'Crop2D')(G)
        
        G = Model(inputs = [real_img, latent],
                    outputs = [G],
                    name = 'Generator')
    ...
    def build_adversarial_model(self):
        ...
        real_img = Input(shape=(self.img_rows, self.img_cols, self.channel))
        latent = Input(shape=(self.latent_dim,))
        AM = self.models['generator']([real_img,latent])
        AM = self.models['discriminator'](AM)
        # Compile the generator model on the number of specified gpus
        if self.gpus <=1:
            AM = Model(inputs = [real_img,latent],
                        outputs = [AM],
    ...
    def train(self,
        ...
        for i in range(train_steps):
            for k in range(train_rate[0]):
                ...
                images_real_d = x_train[np.random.randint(0,
                    x_train.shape[0], size=batch_size), :, :, :]

                images_real_g = x_train[np.random.randint(0,
                    x_train.shape[0], size=batch_size), :, :, :]
                
                # Generate fake images from generator
                noise = np.random.normal(loc=0., scale=1., size=[batch_size, self.latent_dim])
                images_fake = self.models['generator'].predict([images_real_g,noise])
                
                d_loss_real = self.models['discriminator_model'].train_on_batch(images_real_d, y_real)
            ...
            for j in range(train_rate[1]):
                ...
                images_real_g = x_train[np.random.randint(0,
                    x_train.shape[0], size=batch_size), :, :, :]
                a_loss = self.models['adversarial_model'].train_on_batch([images_real_g,noise], y_lie)
            ...
            logger.update(self, tracked, x_samples = images_real_d[:8])
