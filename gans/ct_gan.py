...
class CTGAN(object):
    ...
    def build_discriminator(self):
        ...
        def discriminator_block(D, depth, kernel_size, stride):
            ...
            D.add(Dropout(.5, name = 'DR_D%i'%(i+2)))
    ...
    def ct_loss(self, y_true, y_pred, D_diffs, D_diffs_):
        return K.mean(K.relu(K.square(D_diffs)*2+\
                    K.mean(K.square(D_diffs_),axis=np.arange(1, len(D_diffs_.shape)))*.2 - self.M))
                      
    def discriminator_last2_outputs(self):
        model = self.models['discriminator']
        return Model(inputs = model.inputs,
                     outputs = [model.get_layer('Dense_D2').output,
                                model.get_layer('Dense_D1').output])
    ...
    def build_discriminator_model(self):
        ...
        disc2 = self.discriminator_last2_outputs()
        real1, real1_ = disc2(real_img)
        real2, real2_ = disc2(real_img)
        diffs = Subtract()([real1,real2])
        diffs_ = Subtract()([real1_,real2_])
        ...
        partial_ct_loss = partial(self.ct_loss, D_diffs = diffs, D_diffs_ = diffs_)
        ...
        DM.compile(loss=[self.wasserstein_loss,self.wasserstein_loss,partial_gp_loss, partial_ct_loss],
